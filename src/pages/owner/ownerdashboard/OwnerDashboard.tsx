...
const OwnerDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchShops = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/v1/shops/myshops', {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        });
        let shopsData = response.data;
        if (shopsData && typeof shopsData === 'object' && !Array.isArray(shopsData)) {
    ...
    <div className="min-h-screen bg-gray-50 font-sans antialiased">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-1">Manage your rental business from one place</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
          ...
          >
            <div>
              ...
              <p className="text-xl font-semibold text-gray-800">View Requests</p>
            </div>
            <div className="bg-emerald-50 rounded-full p-3 group-hover:bg-emerald-100 transition">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-bold text-gray-900">Your Rental Shops</h2>
            {shops.length > 0 && (
              <Link
                to="/owner/shops/all"
          ...
          {shops.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {shops.map((shop) => (
                <div key={shop.id} className="bg-white rounded-2xl shadow-md hover:shadow-xl transition overflow-hidden">
                  <div className="p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-gray-100">
                      <div className="bg-gray-50 rounded-xl p-3 text-center">
                        ...
                        <p className="text-2xl font-bold text-gray-800">{shop.totalBikes}</p>
                      </div>
                      <div className="bg-gray-50 rounded-xl p-3 text-center">
                        <p className="text-xs text-gray-500">Active Bookings</p>
                        <p className="text-2xl font-bold text-gray-800">{shop.totalBookings}</p>
                      </div>
                    </div>
        ...
        <div className="mt-8 bg-white rounded-2xl shadow-md p-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3 text-sm text-gray-600">
              ...
              <span>No recent bookings yet. Your shops will appear here once customers book.</span>
            </div>
          </div>
        </div>
      </div>
